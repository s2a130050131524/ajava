...
<html>
    ...
    <body>
        ...
        <sql:setDataSource var="con" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/jdbc" user="root" password="vaishali"/>
        <sql:update dataSource="${con}" var="count">
            insert into books_detail(id,bookname,author) values(?,?,?); 
        <sql:param value="${id}"/>
        <sql:param value="${bookname}"/>
        <sql:param value="${author}"/>
        </sql:update>
        <sql:query dataSource="${con}" var="rs">select * from books_detail;</sql:query>
